    ...
    def move(self, full_map):
        ...
        try:
            ...
            while full_map.loc[future_position[0], future_position[1]] == '#':
                ...
                if possible_direction == '^' and (full_map.loc[:path_with_blocker[0], path_with_blocker[1]].str.contains(r'\^')).any():
                    self.new_blockers += 1
                if possible_direction == '>' and (full_map.loc[path_with_blocker[0], path_with_blocker[1]:].str.contains(r'\>')).any():
                    self.new_blockers += 1
                if possible_direction == 'v' and (full_map.loc[path_with_blocker[0]:, path_with_blocker[1]].str.contains(r'\v')).any():
                    self.new_blockers += 1
                if possible_direction == '<' and (full_map.loc[path_with_blocker[0], :path_with_blocker[1]].str.contains(r'\<')).any():
                    self.new_blockers += 1
                
                #if full_map.loc[path_with_blocker[0], path_with_blocker[1]] == possible_direction:
                 #   self.new_blockers += 1
